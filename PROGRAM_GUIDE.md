...
3. เลือก Inverter ที่มีกำลัง DC ถูกจัดเข้าแล้วน้อยที่สุด
4. ภายใน Inverter นั้น เลือก MPPT ที่มีจำนวน input ใช้น้อยที่สุด
5. ตรวจ `inputs_per_mppt`, `max_i_mppt_a` และ `max_isc_mppt_a`
6. ถ้าไม่มี slot ที่ผ่าน จะกำหนดเป็น `UNASSIGNED` และสถานะ `FAIL`
